...
namespace fcf {
  ...
  struct TypeInfo{
    ...
    typename TSpecificator::UniversalCallType getSpecificator() const {
      const unsigned int specificatorIndex = TypeId<TSpecificator>().index();
      std::map<unsigned int, SpecificatorTypeInfo>::const_iterator it = specificators.find(specificatorIndex);
      if (it != specificators.end()) {
        return (typename TSpecificator::UniversalCallType)it->second.resolve;
      }
      return (typename TSpecificator::UniversalCallType)0;
    }
  };
