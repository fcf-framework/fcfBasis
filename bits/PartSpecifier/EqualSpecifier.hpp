#ifndef ___FCF_BASIS__BITS__PART_SPECIFIER__EQUAL_SPECIFIER_HPP___
#define ___FCF_BASIS__BITS__PART_SPECIFIER__EQUAL_SPECIFIER_HPP___
...
namespace fcf{
  ...
  struct TypeImpl<Ty, EqualSpecifier> {
    ...
    inline bool call(const Ty* a_leftValue, const Ty* a_rightValue) {
      return Type<Ty, EqualSpecifier>()(a_leftValue, a_rightValue);
    }
    ...
    inline Variant universalCall(const Ty* a_object, const Variant* a_argv, size_t a_argc) const {
      ...
      if (Type<Ty>().index() != a_argv[0].getDataTypeIndex()){
        ...
        return Variant(Type<Ty, EqualSpecifier>().call(a_object, &right));
      }
      const Ty* ptr = (const Ty*)a_argv->ptr();
      if (!ptr) {
        throw MathEmptyArgumentException(__FILE__, __LINE__, "==", 2);
      }
      return Variant(Type<Ty, EqualSpecifier>().call(a_object, ptr));
    }
...
#endif // #ifndef ___FCF_BASIS__BITS__PART_SPECIFIER__EQUAL_SPECIFIER_HPP___
