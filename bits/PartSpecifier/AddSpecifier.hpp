#ifndef ___FCF_BASIS__BITS__PART_SPECIFIER__ADD_SPECIFIER_HPP___
#define ___FCF_BASIS__BITS__PART_SPECIFIER__ADD_SPECIFIER_HPP___
...
namespace fcf{
  ...
  struct Type<Ty, AddSpecifier> {
  };

  template <typename Ty>
  struct TypeImpl<Ty, AddSpecifier> {
    ...
    inline void call(Ty* a_destination, const Ty* a_leftValue, const Ty* a_rightValue) {
      Type<Ty, AddSpecifier>()(a_destination, a_leftValue, a_rightValue);
    }
    ...
    inline Variant universalCall(Ty* a_object, Variant* a_argv, size_t a_argc) const {
      ...
      if (Type<Ty>().index() != a_argv[0].getDataTypeIndex() ||
        ...
        Type<Ty, AddSpecifier>().call(a_object, &left, &right);
      } else {
        ...
        Type<Ty, AddSpecifier>().call(a_object, leftPtr, rightPtr);
      }
      return Variant();
    }
...
#endif // #ifndef ___FCF_BASIS__BITS__PART_SPECIFIER__ADD_SPECIFIER_HPP___
